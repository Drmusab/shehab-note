...
const renderDoc = (element: HTMLElement, currentPage: number) => {
    ...
    if (typeElement.value === "2") {
        ...
        if (window.shehab.storage[Constants.LOCAL_HISTORY].type !== 2) {
            opElement.value = "all";
        }
    ...
    window.shehab.storage[Constants.LOCAL_HISTORY].notebookId = notebookElement.value;
    window.shehab.storage[Constants.LOCAL_HISTORY].type = parseInt(typeElement.value);
    window.shehab.storage[Constants.LOCAL_HISTORY].operation = opElement.value;
    setStorageVal(Constants.LOCAL_HISTORY, window.shehab.storage[Constants.LOCAL_HISTORY]);
    fetchPost("/api/history/searchHistory", {
        ...
        pageElement.textContent = `${window.shehab.languages.pageCountAndHistoryCount.replace("${x}", response.data.pageCount).replace("${y}", response.data.totalCount || 1)}`;
        pageElement.classList.remove("fn__none");
        if (response.data.histories.length === 0) {
            listElement.innerHTML = `<li class="b3-list--empty">${window.shehab.languages.emptyContent}</li>`;
            return;
        }
        let logsHTML = "";
...
const renderRepoItem = (response: IWebSocketData, element: Element, type: string) => {
    if (response.data.snapshots.length === 0) {
        element.lastElementChild.innerHTML = `<li class="b3-list--empty">${window.shehab.languages.emptyContent}</li>`;
        return;
    }
    let actionHTML = "";
    /// #if MOBILE
...
<span class="b3-list-item__action" data-type="uploadSnapshot">
    ...
    <span class="fn__space"></span>
    ${window.shehab.languages.upload}
</span>
<span class="fn__flex-1"></span>
<span class="b3-list-item__action" data-type="rollback">
    <svg><use xlink:href="#iconUndo"></use></svg>
    <span class="fn__space"></span>
    ${window.shehab.languages.rollback}
</span>
<span class="fn__flex-1"></span>
...
<span class="b3-list-item__action" data-type="genTag">
    ...
    <span class="fn__space"></span>
    ${window.shehab.languages.tagSnapshot}
</span>
<span class="fn__flex-1"></span>
<span class="b3-list-item__action" data-type="rollback">
    <svg><use xlink:href="#iconUndo"></use></svg>
...
<span class="fn__flex-1"></span>`;
    ...
    /// #else
    if (type === "getCloudRepoTagSnapshots") {
        actionHTML = `<span class="b3-list-item__action b3-tooltips b3-tooltips__w" data-type="downloadSnapshot" aria-label="${window.shehab.languages.download}"><svg><use xlink:href="#iconDownload"></use></svg></span>
<span class="b3-list-item__action b3-tooltips b3-tooltips__w" data-type="downloadRollback" aria-label="${window.shehab.languages.downloadRollback}"><svg><use xlink:href="#iconUndo"></use></svg></span>
<span class="b3-list-item__action b3-tooltips b3-tooltips__w" data-type="removeCloudRepoTagSnapshot" aria-label="${window.shehab.languages.remove}"><svg><use xlink:href="#iconTrashcan"></use></svg></span>`;
    } else if (type === "getCloudRepoSnapshots") {
        actionHTML = `<span class="b3-list-item__action b3-tooltips b3-tooltips__w" data-type="downloadSnapshot" aria-label="${window.shehab.languages.download}"><svg><use xlink:href="#iconDownload"></use></svg></span>
<span class="b3-list-item__action b3-tooltips b3-tooltips__w" data-type="downloadRollback" aria-label="${window.shehab.languages.downloadRollback}"><svg><use xlink:href="#iconUndo"></use></svg></span>`;
    } else if (type === "getRepoTagSnapshots") {
        actionHTML = `<span class="b3-list-item__action b3-tooltips b3-tooltips__w" data-type="uploadSnapshot" aria-label="${window.shehab.languages.upload}"><svg><use xlink:href="#iconUpload"></use></svg></span>
<span class="b3-list-item__action b3-tooltips b3-tooltips__w" data-type="rollback" aria-label="${window.shehab.languages.rollback}"><svg><use xlink:href="#iconUndo"></use></svg></span>
<span class="b3-list-item__action b3-tooltips b3-tooltips__w" data-type="removeRepoTagSnapshot" aria-label="${window.shehab.languages.remove}"><svg><use xlink:href="#iconTrashcan"></use></svg></span>`;
    } else if (type === "getRepoSnapshots") {
        actionHTML = `<span class="b3-list-item__action b3-tooltips b3-tooltips__w" data-type="genTag" aria-label="${window.shehab.languages.tagSnapshot}"><svg><use xlink:href="#iconTags"></use></svg></span>
<span class="b3-list-item__action b3-tooltips b3-tooltips__w" data-type="rollback" aria-label="${window.shehab.languages.rollback}"><svg><use xlink:href="#iconUndo"></use></svg></span>`;
    }
    /// #endif
    let repoHTML = "";
...
${window.shehab.languages.fileCount} ${item.count}<span class="fn__space"></span>`;
...
const renderRepo = (element: Element, currentPage: number) => {
    ...
    if (selectValue === "getRepoTagSnapshots" || selectValue === "getCloudRepoTagSnapshots") {
        ...
        fetchPost(`/api/repo/${selectValue}`, {page: currentPage}, (response) => {
            ...
            pageBtn.setAttribute("data-totalpage", (response.data.pageCount || 1).toString());
            pageElement.textContent = `${window.shehab.languages.pageCountAndSnapshotCount.replace("${x}", response.data.pageCount).replace("${y}", response.data.totalCount || 1)}`;
            pageElement.classList.remove("fn__none");
            renderRepoItem(response, element, selectValue);
        });
    }
...
const renderRmNotebook = (element: HTMLElement) => {
    ...
    fetchPost("/api/history/getNotebookHistory", {}, (response) => {
        if (response.data.histories.length === 0) {
            element.innerHTML = `<li class="b3-list--empty">${window.shehab.languages.emptyContent}</li>`;
            return;
        }
        let logsHTML = "";
    ...
    <span class="fn__space"></span>
    <span class="b3-list-item__action b3-tooltips b3-tooltips__w" data-type="rollback" aria-label="${window.shehab.languages.rollback}">
        <svg><use xlink:href="#iconUndo"></use></svg>
    </span>
</li>`;
                });
...
export const openHistory = (app: App) => {
    ...
    const localHistory = window.shehab.storage[Constants.LOCAL_HISTORY];
    let notebookSelectHTML = `<option value='%' ${localHistory.notebookId === "%" ? "selected" : ""}>${window.shehab.languages.allNotebooks}</option>`;
    ...
    <div class="layout-tab-bar fn__flex" ${isMobile() ? "" : 'style="border-radius: var(--b3-border-radius-b) var(--b3-border-radius-b) 0 0"'}>
        <div data-type="doc" class="item item--full item--focus"><span class="fn__flex-1"></span><span class="item__text">${window.shehab.languages.fileHistory}</span><span class="fn__flex-1"></span></div>
        <div data-type="notebook" style="min-width: 160px" class="item item--full"><span class="fn__flex-1"></span><span class="item__text">${window.shehab.languages.removedNotebook}</span><span class="fn__flex-1"></span></div>
        <div data-type="repo" class="item item--full"><span class="fn__flex-1"></span><span class="item__text">${window.shehab.languages.dataSnapshot}</span><span class="fn__flex-1"></span></div>
    </div>
    <div class="fn__flex-1 fn__flex" id="historyContainer">
        <div data-type="doc" class="history__repo fn__block" data-init="true">
            <div class="history__action">
                <div class="block__icons">
                    <span data-type="docprevious" class="block__icon block__icon--show b3-tooltips b3-tooltips__e" disabled="disabled" aria-label="${window.shehab.languages.previousLabel}"><svg><use xlink:href='#iconLeft'></use></svg></span>
                    <button class="b3-button b3-button--text ft__selectnone" data-type="jumpHistoryPage" data-totalpage="1">1</button>
                    <span data-type="docnext" class="block__icon block__icon--show b3-tooltips b3-tooltips__e" disabled="disabled" aria-label="${window.shehab.languages.nextLabel}"><svg><use xlink:href='#iconRight'></use></svg></span>
                    <span class="fn__space"></span>
                    <span class="ft__on-surface fn__flex-shrink ft__selectnone fn__none">${window.shehab.languages.pageCountAndHistoryCount}</span>
                    <span class="fn__space"></span>
                    ...
                    <select data-type="typeselect" class="b3-select ${isMobile() ? "fn__size96" : "fn__size200"}">
                        <option value="0" ${localHistory.type === 0 ? "selected" : ""}>${window.shehab.languages.docName}</option>
                        <option value="1" ${localHistory.type === 1 ? "selected" : ""}>${window.shehab.languages.docNameAndContent}</option>
                        <option value="2" ${localHistory.type === 2 ? "selected" : ""}>${window.shehab.languages.assets}</option>
                    </select>
                    <span class="fn__space"></span>
                    <select data-type="opselect" class="b3-select${isMobile() ? " fn__size96" : ""}">
                        <option value="all" ${localHistory.operation === "all" ? "selected" : ""}>${window.shehab.languages.allOp}</option>
                        <option value="clean" ${localHistory.operation === "clean" ? "selected" : ""}>${window.shehab.languages.historyClean}</option>
                        <option value="update" ${localHistory.operation === "update" ? "selected" : ""}>${window.shehab.languages.historyUpdate}</option>
                        <option value="delete" ${localHistory.operation === "delete" ? "selected" : ""}>${window.shehab.languages.historyDelete}</option>
                        <option value="format" ${localHistory.operation === "format" ? "selected" : ""}>${window.shehab.languages.historyFormat}</option>
                        <option value="sync" ${localHistory.operation === "sync" ? "selected" : ""}>${window.shehab.languages.historySync}</option>
                        <option value="replace" ${localHistory.operation === "replace" ? "selected" : ""}>${window.shehab.languages.historyReplace}</option>
                        <option value="outline" ${localHistory.operation === "outline" ? "selected" : ""}>${window.shehab.languages.historyOutline}</option>
                    </select>
                    <span class="fn__space"></span>
                    ...
                    <span class="fn__space"></span>
                    <button data-type="rebuildIndex" class="b3-button b3-button--outline">${window.shehab.languages.rebuildIndex}</button>
                </div>
            </div>
            <div class="fn__flex fn__flex-1 history__panel">
                <ul class="b3-list b3-list--background history__side" ${isMobile() ? "" : `style="width: ${localHistory.sideWidth}"`}>
                    <li class="b3-list--empty">${window.shehab.languages.emptyContent}</li>
                </ul>
        ...
        <div data-type="repo" class="fn__none history__repo">
            <div class="history__action">
                <div class="block__icons">
                    <span data-type="previous" class="block__icon block__icon--show b3-tooltips b3-tooltips__e" disabled="disabled" aria-label="${window.shehab.languages.previousLabel}"><svg><use xlink:href='#iconLeft'></use></svg></span>
                    <button class="b3-button b3-button--text ft__selectnone" data-type="jumpRepoPage" data-totalpage="1">1</button>
                    <span data-type="next" class="block__icon block__icon--show b3-tooltips b3-tooltips__e" disabled="disabled" aria-label="${window.shehab.languages.nextLabel}"><svg><use xlink:href='#iconRight'></use></svg></span>
                    <span class="fn__space"></span>
                    <span class="ft__on-surface fn__flex-shrink ft__selectnone fn__none">${window.shehab.languages.pageCountAndSnapshotCount}</span>
                    <span class="fn__space"></span>
                    <div class="fn__flex-1"></div>
                    <select class="b3-select ${isMobile() ? "fn__size96" : "fn__size200"}">
                        <option value="getRepoSnapshots">${window.shehab.languages.localSnapshot}</option>
                        <option value="getRepoTagSnapshots">${window.shehab.languages.localTagSnapshot}</option>
                        <option value="getCloudRepoSnapshots">${window.shehab.languages.cloudSnapshot}</option>
                        <option value="getCloudRepoTagSnapshots">${window.shehab.languages.cloudTagSnapshot}</option>
                    </select>
                    <span class="fn__space"></span>
                    <button class="b3-button b3-button--outline" disabled data-type="compare">${window.shehab.languages.compare}</button>
                    <span class="fn__space"></span>
                    <button class="b3-button b3-button--outline" data-type="genRepo">
                        <svg><use xlink:href="#iconAdd"></use></svg>${window.shehab.languages.createSnapshot}
                    </button>
                </div>    
            </div>
    ...
    if (isMobile()) {
        openModel({
            html: contentHTML,
            icon: "iconHistory",
            title: window.shehab.languages.dataHistory,
            bindEvent(element) {
...
const bindEvent = (app: App, element: Element, dialog?: Dialog) => {
    ...
    element.addEventListener("click", (event) => {
        ...
        while (target && !target.isEqualNode(element)) {
            ...
            if (target.classList.contains("item")) {
                ...
                break;
            } else if (target.classList.contains("b3-list-item__action") && type === "rollback" && !window.shehab.config.readonly) {
                ...
                if (dataType === "notebook") {
                    ...
                    name = window.shehab.languages.workspaceData;
                ...
                const confirmTip = window.shehab.languages.rollbackConfirm.replace("${name}", name)
                    .replace("${time}", time);
                confirmDialog("⚠️ " + window.shehab.languages.rollback, confirmTip, () => {
                    if (dataType === "assets") {
                        ...
                        fetchPost("/api/history/getHistoryItems", {
                            ...
                            response.data.items.forEach((docItem: {
                                ...
                                if (docItem.op === "clean") {
                                    ...
                                    ariaLabel = window.shehab.languages.historyClean;
                                } else if (docItem.op === "update") {
                                    chipClass += "b3-chip--info ";
                                    ariaLabel = window.shehab.languages.historyUpdate;
                                } else if (docItem.op === "delete") {
                                    chipClass += "b3-chip--error ";
                                    ariaLabel = window.shehab.languages.historyDelete;
                                } else if (docItem.op === "format") {
                                    chipClass += "b3-chip--pink ";
                                    ariaLabel = window.shehab.languages.historyFormat;
                                } else if (docItem.op === "sync") {
                                    chipClass += "b3-chip--success ";
                                    ariaLabel = window.shehab.languages.historySync;
                                } else if (docItem.op === "replace") {
                                    chipClass += "b3-chip--secondary ";
                                    ariaLabel = window.shehab.languages.historyReplace;
                                } else if (docItem.op === "outline") {
                                    chipClass += "b3-chip--warning ";
                                    ariaLabel = window.shehab.languages.historyOutline;
                                }
    ...
    <span class="fn__space"></span>
    <span class="b3-list-item__action ariaLabel" data-type="rollback" data-position="6south" aria-label="${window.shehab.languages.rollback}">
        <svg><use xlink:href="#iconUndo"></use></svg>
    </span>
</li>`;
                            });
                ...
                const genRepoDialog = new Dialog({
                    title: window.shehab.languages.snapshotMemo,
                    content: `<div class="b3-dialog__content">
    <textarea class="b3-text-field fn__block" placeholder="${window.shehab.languages.snapshotMemoTip}"></textarea>
</div>
<div class="b3-dialog__action">
    ...
    <button class="b3-button b3-button--text">${window.shehab.languages.confirm}</button>
                ...
                confirmDialog(window.shehab.languages.deleteOpConfirm, `${window.shehab.languages.confirmDelete} <i>${tag}</i>?`, () => {
                ...
                event.preventDefault();
                break;
            } else if (type === "downloadRollback" && !window.shehab.config.readonly) {
                confirmDialog("⚠️ " + window.shehab.languages.downloadRollback, window.shehab.languages.rollbackConfirm.replace("${name}", window.shehab.languages.workspaceData)
                ...
                const genTagDialog = new Dialog({
                    title: window.shehab.languages.tagSnapshot,
                    content: `<div class="b3-dialog__content">
    <input class="b3-text-field fn__block" value="${dayjs().format("YYYYMMDDHHmmss")}" placeholder="${window.shehab.languages.tagSnapshotTip}">
</div>
<div class="b3-dialog__action">
    <button class="b3-button b3-button--cancel">${window.shehab.languages.cancel}</button><div class="fn__space"></div>
    <button class="b3-button b3-button--text">${window.shehab.languages.tagSnapshot}</button><div class="fn__space"></div>
    <button class="b3-button b3-button--text">${window.shehab.languages.tagSnapshotUpload}</button>
</div>`,
                    width: isMobile() ? "92vw" : "520px",
                });
                ...
                if (totalPage > 1) {
                    confirmDialog(
                        window.shehab.languages.jumpToPage.replace("${x}", totalPage),
