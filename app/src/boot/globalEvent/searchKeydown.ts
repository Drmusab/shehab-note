...
export const searchKeydown = (app: App, event: KeyboardEvent) => {
    ...
    if (searchType === "doc" && matchHotKey(window.shehab.config.keymap.general.newFile.custom, event)) {
        if (config.method === 0) {
    ...
    const assetLocal = window.shehab.storage[Constants.LOCAL_SEARCHASSET] as ISearchAssetOption;
    ...
    if (searchType !== "asset") {
        if (matchHotKey(window.shehab.config.keymap.editor.general.insertRight.custom, event)) {
            openSearchEditor({
        ...
        if (matchHotKey(window.shehab.config.keymap.editor.general.copyBlockRef.custom, event)) {
    ...
    if (event.key === "Enter") {
        if (searchType !== "asset") {
            ...
            /// #if !BROWSER
            useShell("showItemInFolder", path.join(window.shehab.config.system.dataDir, currentList.lastElementChild.getAttribute("aria-label")));
            /// #endif
        }
        return true;
    }
