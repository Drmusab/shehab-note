...
const matchKeymap = (keymap: Config.IKeys, key1: "general" | "editor", key2?: "general" | "insert" | "heading" | "list" | "table") => {
    if (key1 === "general") {
        if (!window.shehab.config.keymap[key1]) {
            ...
            /// #endif
            window.shehab.config.keymap[key1] = keymap as Config.IKeymapGeneral;
            return false;
        }
    } else {
        if (!window.shehab.config.keymap[key1]) {
        ...
        if (!window.shehab.config.keymap[key1][key2]) {
            ...
            /// #endif
            (window.shehab.config.keymap[key1][key2] as Config.IKeymapEditor[typeof key2]) = keymap as Config.IKeymapEditor[typeof key2];
            return false;
        }
    }
    let match = true;
    Object.keys(keymap).forEach(key => {
        if (key1 === "general") {
            if (!window.shehab.config.keymap[key1][key] || window.shehab.config.keymap[key1][key].default !== keymap[key].default) {
                ...
                /// #endif
                match = false;
                window.shehab.config.keymap[key1][key] = keymap[key];
            }
        } else {
            if (!window.shehab.config.keymap[key1][key2][key] || window.shehab.config.keymap[key1][key2][key].default !== keymap[key].default) {
                ...
                /// #endif
                match = false;
                window.shehab.config.keymap[key1][key2][key] = keymap[key];
            }
        }
    });
    return match;
...
export const correctHotkey = (app: App) => {
    ...
    if (!window.shehab.config.readonly &&
        ...
        fetchPost("/api/setting/setKeymap", {
            data: window.shehab.config.keymap
        }, () => {
            /// #if !BROWSER
...
export const filterHotkey = (event: KeyboardEvent, app: App) => {
    ...
    if (!event.altKey && !event.shiftKey && isOnlyMeta(event)) {
        if ((isMac() ? event.key === "Meta" : event.key === "Control") || isOnlyMeta(event)) {
            window.shehab.ctrlIsPressed = true;
            if ((event.key === "Meta" || event.key === "Control") &&
                window.shehab.config.editor.floatWindowMode === 1 && !event.repeat) {
                showPopover(app);
            }
        } else {
    ...
    if (!event.altKey && event.shiftKey && isNotCtrl(event)) {
        if (event.key === "Shift") {
            window.shehab.shiftIsPressed = true;
            if (!event.repeat) {
    ...
    if (event.altKey && !event.shiftKey && isNotCtrl(event)) {
        if (event.key === "Alt") {
            window.shehab.altIsPressed = true;
        } else {
