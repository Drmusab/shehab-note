...
export const account = {
    ...
    genHTML: (onlyPayHTML = false) => {
        ...
        if (isIOS) {
            // 已付费
            if (window.shehab.user?.userSiYuanOneTimePayStatus === 1) {
                payHTML = `<button class="b3-button b3-button--big" data-action="iOSPay" data-type="subscribe">
    <svg><use xlink:href="#iconVIP"></use></svg>${window.shehab.languages.account4}
</button>`;
            } else {
                payHTML = `<button class="b3-button b3-button--big" data-action="iOSPay" data-type="subscribe">
    <svg><use xlink:href="#iconVIP"></use></svg>${window.shehab.languages.account10}
</button>
<div class="fn__hr"></div>
<button class="b3-button b3-button--success" data-action="iOSPay" data-type="function">
    <svg><use xlink:href="#iconVIP"></use></svg>${window.shehab.languages.onepay}
</button>`;
            }
        } else {
            payHTML = `<a class="b3-button b3-button--big" href="${getIndexURL("pricing.html")}" target="_blank">
    <svg><use xlink:href="#iconVIP"></use></svg>${window.shehab.languages[window.shehab.user?.userSiYuanOneTimePayStatus === 1 ? "account4" : "account1"]}
</a>`;
        }
        payHTML += `<div class="fn__hr--b"></div>
<span class="b3-chip b3-chip--primary b3-chip--hover${(window.shehab.user && window.shehab.user.userSiYuanSubscriptionStatus === 2) ? " fn__none" : ""}" id="trialSub">
    <svg class="ft__secondary"><use xlink:href="#iconVIP"></use></svg>
    ${window.shehab.languages.freeSub}
</span>
<div class="fn__hr${(window.shehab.user && window.shehab.user.userSiYuanSubscriptionStatus === 2) ? " fn__none" : ""}"></div>
<a href="${getCloudURL("sponsor")}" target="_blank" class="${isIOS ? "fn__none " : ""}b3-chip b3-chip--pink b3-chip--hover">
    ...
    ${window.shehab.languages.sponsor}
</a>
<div class="fn__hr--b"></div>
<div class="fn__flex-1"></div>
<div>
${window.shehab.languages.accountSupport1}
</div>
<div class="fn__hr--b"></div>
<div>
${window.shehab.languages.accountSupport2}
</div>`;
        if (onlyPayHTML) {
...
<div class="fn__flex-1 fn__hr--b"></div>`;
        ...
        if (window.shehab.user) {
            let userTitlesHTML = "";
            if (window.shehab.user.userTitles.length > 0) {
                userTitlesHTML = '<div class="b3-chips" style="position: absolute">';
                window.shehab.user.userTitles.forEach((item) => {
   ...
   <input class="b3-text-field fn__block b3-form__icon-input" style="padding-right: 44px;" placeholder="${window.shehab.languages.activationCodePlaceholder}">
   <button id="activationCode" class="b3-button b3-button--text" style="position: absolute;right: 0;top: 0;">${window.shehab.languages.confirm}</button>
</div>`;
...
<div class="ft__on-surface ft__smaller">
    ${window.shehab.languages.account6} 
    ${Math.max(0, Math.floor((window.shehab.user.userSiYuanProExpireTime - new Date().getTime()) / 1000 / 60 / 60 / 24))} 
    ${window.shehab.languages.day} 
    ${isIOS ? `<a href="javascript:void(0)" data-action="iOSPay" data-type="subscribe">${window.shehab.languages.clickMeToRenew}</a>` : `<a href="${getCloudURL("subscribe/siyuan")}" target="_blank">${window.shehab.languages.clickMeToRenew}</a>`}
</div>`;
                if (window.shehab.user.userSiYuanOneTimePayStatus === 1) {
                    subscriptionHTML = `<div class="b3-chip b3-chip--success"><svg><use xlink:href="#iconVIP"></use></svg>${window.shehab.languages.account7}</div>
<div class="fn__hr--b"></div>`;
                }
                if (window.shehab.user.userSiYuanSubscriptionPlan === 2) {
                    // 订阅试用
                    subscriptionHTML += `<div class="b3-chip b3-chip--primary"><svg><use xlink:href="#iconVIP"></use></svg>${window.shehab.languages.account3}</div>
${renewHTML}<div class="fn__hr--b"></div>`;
                } else {
                    // 年费
                    subscriptionHTML += `<div class="b3-chip b3-chip--primary"><svg class="ft__secondary"><use xlink:href="#iconVIP"></use></svg>${window.shehab.languages.account8}</div>
${renewHTML}<div class="fn__hr--b"></div>`;
                }
                if (window.shehab.user.userSiYuanOneTimePayStatus === 0) {
...
<div class="config-account__center">
    <div class="config-account__bg">
        <div class="config-account__cover" style="background-image: url(${window.shehab.user.userHomeBImgURL})"></div>
        <a href="${getCloudURL("settings/avatar")}" class="config-account__avatar" style="background-image: url(${window.shehab.user.userAvatarURL})" target="_blank"></a>
        <h1 class="config-account__name">
            <a target="_blank" class="fn__a" href="${getCloudURL("member/" + window.shehab.user.userName)}">${window.shehab.user.userName}</a>
            <span class="ft__on-surface ft__smaller">${0 === window.shehab.config.cloudRegion ? "ld246.com" : "liuyun.io"}</span>
        </h1>
        ${userTitlesHTML}
    </div>
    <div class="config-account__info">
        <div class="fn__flex">
            <a class="b3-button b3-button--text${isIOS ? " fn__none" : ""}" href="${getCloudURL("settings")}" target="_blank">${window.shehab.languages.manage}</a>
            <span class="fn__space${isIOS ? " fn__none" : ""}"></span>
            <button class="b3-button b3-button--cancel" id="logout">
                ${window.shehab.languages.logout}
            </button>
            <span class="fn__space"></span>
            <button class="b3-button b3-button--cancel${window.shehab.config.system.container === "ios" ? "" : " fn__none"}" id="deactivateUser">
                ${window.shehab.languages.deactivateUser}
            </button>
            <span class="fn__flex-1"></span>
            <button class="b3-button b3-button--cancel b3-tooltips b3-tooltips__n" id="refresh" aria-label="${window.shehab.languages.refresh}">
        ...
        <div class="fn__flex">  
            <label>
                ${window.shehab.languages.accountDisplayTitle}
                <input class="b3-switch fn__flex-center" id="displayTitle" type="checkbox"${window.shehab.config.account.displayTitle ? " checked" : ""}/>
            </label>
            <div class="fn__flex-1"></div>
            <label>
                ${window.shehab.languages.accountDisplayVIP}
                <input class="b3-switch fn__flex-center" id="displayVIP" type="checkbox"${window.shehab.config.account.displayVIP ? " checked" : ""}/>
            </label>
        </div>
    </div>
</div>
...
<div class="b3-form__space config-account__center">
    <div class="config-account__form" id="form1">
        <div class="b3-form__icon">
            ...
            <input id="userName" class="b3-text-field fn__block b3-form__icon-input" placeholder="${window.shehab.languages.accountName}">
        </div>
        <div class="fn__hr--b"></div>
        <div class="b3-form__icon">
            <svg class="b3-form__icon-icon"><use xlink:href="#iconLock"></use></svg>
            <input type="password" id="userPassword" class="b3-text-field b3-form__icon-input fn__block" placeholder="${window.shehab.languages.password}">
        </div>
        <div class="fn__hr--b"></div>
        <div class="b3-form__icon">
            ...
            <select class="b3-select b3-form__icon-input fn__block" id="cloudRegion">
                <option value="0"${window.shehab.config.cloudRegion === 0 ? " selected" : ""}>${window.shehab.languages.cloudRegionChina}</option>
                <option value="1"${window.shehab.config.cloudRegion === 1 ? " selected" : ""}>${window.shehab.languages.cloudRegionNorthAmerica}</option>
            </select>
        </div>
        <div class="b3-form__img fn__none">
            <div class="fn__hr--b"></div>
            <img id="captchaImg" class="fn__pointer" style="top: 17px;height:26px">
            <input id="captcha" class="b3-text-field fn__block" placeholder="${window.shehab.languages.captcha}">
        </div>
        <div class="fn__hr--b"></div>
        <label class="ft__smaller ft__on-surface fn__flex">
            ...
            <span class="fn__space"></span>
            <span>${window.shehab.languages.accountTip}</span>
        </label>
        <div class="fn__hr--b"></div>
        <button id="login" disabled class="b3-button fn__block">${window.shehab.languages.login}</button>
        <div class="fn__hr--b"></div>
        <div class="ft__center">
            <a href="${getCloudURL("forget-pwd")}" class="b3-button b3-button--cancel" target="_blank">${window.shehab.languages.forgetPassword}</a>
            <span class="fn__space${window.shehab.config.system.container === "ios" ? " fn__none" : ""}"></span>
            <a href="${getCloudURL("register")}" class="b3-button b3-button--cancel${window.shehab.config.system.container === "ios" ? " fn__none" : ""}" target="_blank">${window.shehab.languages.register}</a>
        </div>
    </div>
    <div class="fn__none config-account__form" id="form2">
        <div class="b3-form__icon">
            <svg class="b3-form__icon-icon"><use xlink:href="#iconLock"></use></svg>
            <input id="twofactorAuthCode" class="b3-text-field fn__block b3-form__icon-input" placeholder="${window.shehab.languages.twoFactorCaptcha}">
        </div>
        <div class="fn__hr--b"></div>
        <button id="login2" class="b3-button fn__block">${window.shehab.languages.login}</button>
    </div>
</div>
<div class="config-account__center config-account__center--text${window.shehab.config.system.container === "ios" ? " fn__none" : ""}">
    <div class="fn__flex-1 fn__hr--b"></div>
    ${genSVGBG()}
    <div class="fn__flex-1 fn__hr--b"></div>    
    ${payHTML}
    ...
    bindEvent: (element: Element) => {
        ...
        if (!userNameElement) {
            ...
            refreshElement.addEventListener("click", () => {
                ...
                fetchPost("/api/setting/getCloudUser", {
                    ...
                    showMessage(window.shehab.languages.refreshUser, 3000);
            ...
            element.querySelector("#deactivateUser").addEventListener(getEventName(), () => {
                confirmDialog("⚠️ " + window.shehab.languages.deactivateUser, window.shehab.languages.deactivateUserTip, () => {
                    fetchPost("/api/account/deactivate", {}, () => {
                        window.shehab.user = null;
                        element.innerHTML = account.genHTML();
                        account.bindEvent(element);
                        account.onSetaccount();
                        processSync();
            ...
            element.querySelectorAll("input[type='checkbox']").forEach(item => {
                item.addEventListener("change", () => {
                    fetchPost("/api/setting/setAccount", {
                        ...
                        window.shehab.config.account.displayTitle = response.data.displayTitle;
                        window.shehab.config.account.displayVIP = response.data.displayVIP;
                        account.onSetaccount();
                    });
                });
            });
            ...
            activationCodeElement?.addEventListener("click", () => {
                ...
                fetchPost("/api/account/checkActivationcode", {data: activationCodeInput.value}, (response) => {
                    ...
                    confirmDialog(window.shehab.languages.activationCode, response.msg, () => {
                        if (response.code === 0) {
        ...
        cloudRegionElement.addEventListener("change", () => {
            window.shehab.config.cloudRegion = parseInt(cloudRegionElement.value);
            element.querySelector(".config-account__center--text").innerHTML = account.genHTML(true);
            element.querySelector("#form1").lastElementChild.innerHTML = `<a href="${getCloudURL("forget-pwd")}" class="b3-button b3-button--cancel" target="_blank">${window.shehab.languages.forgetPassword}</a>
<span class="fn__space${window.shehab.config.system.container === "ios" ? " fn__none" : ""}"></span>
<a href="${getCloudURL("register")}" class="b3-button b3-button--cancel${window.shehab.config.system.container === "ios" ? " fn__none" : ""}" target="_blank">${window.shehab.languages.register}</a>`;
        });
        loginBtnElement.addEventListener("click", () => {
            fetchPost("/api/account/login", {
                ...
                cloudRegion: window.shehab.config.cloudRegion,
            }, (data) => {
                let messageId;
                if (data.code === 1) {
    ...
    _afterLogin(userResponse: IWebSocketData, element: Element) {
        ...
        if (element.getAttribute("data-action") === "go-repos") {
            if (needSubscribe("") && 0 === window.shehab.config.sync.provider) {
    ...
    onSetaccount() {
        ...
        if (window.shehab.config.system.container === "ios") {
            return;
        }
        let html = "";
            ...
            if (window.shehab.user) {
                if (window.shehab.user.userSiYuanProExpireTime === -1) { // 终身会员
                    ...
                    if (window.shehab.user.userSiYuanSubscriptionPlan === 2) { // 试用订阅
                        ...
                        html = `<div class="toolbar__item ariaLabel" aria-label="${window.shehab.languages.account10}"><svg class="ft__secondary"><use xlink:href="#iconVIP"></use></svg></div>`;
                    }
                } else if (window.shehab.user.userSiYuanSubscriptionStatus === -1) { // 未订阅
                    html = `<div class="toolbar__item ariaLabel" aria-label="${window.shehab.languages.freeSub}"><svg class="ft__error"><use xlink:href="#iconVIP"></use></svg></div>`;
                }
                if (window.shehab.user.userSiYuanOneTimePayStatus === 1) { // 一次性付费功能特性
                    html += `<div class="toolbar__item ariaLabel" aria-label="${window.shehab.languages.onepay}"><svg class="ft__success"><use xlink:href="#iconVIP"></use></svg></div>`;
                }
            } else { // 未登录
                html = `<div class="toolbar__item ariaLabel" aria-label="${window.shehab.languages.freeSub}"><svg class="ft__error"><use xlink:href="#iconVIP"></use></svg></div>`;
            }
        }
        if (window.shehab.config.account.displayTitle && window.shehab.user) {
            window.shehab.user.userTitles.forEach(item => {
