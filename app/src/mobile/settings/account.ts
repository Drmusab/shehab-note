...
<div class="fn__hr--b"></div>${payHTML}`;
    ...
    openModel({
        title: window.shehab.languages.manage,
        icon: "iconAccount",
                ...
                while (target && target !== modelMainElement) {
                    if (target.getAttribute("data-action") === "iOSPay") {
                        ...
                        fetchPost("/api/setting/logoutCloudUser", {}, () => {
                            ...
                            closePanel();
                            document.getElementById("menuAccount").innerHTML = `<svg class="b3-menu__icon"><use xlink:href="#iconAccount"></use></svg><span class="b3-menu__label">${window.shehab.languages.login}</span>`;
                            processSync();
                        });
                        event.preventDefault();
                        event.stopPropagation();
                        ...
                        const dialog = new Dialog({
                            title: "⚠️ " + window.shehab.languages.deactivateUser,
                            width: "92vw",
                        ...
                        fetchPost("/api/setting/getCloudUser", {
                            ...
                            if (window.shehab.user) {
                                menuAccountElement.innerHTML = `<img class="b3-menu__icon" src="${window.shehab.user.userAvatarURL}"/><span class="b3-menu__label">${window.shehab.user.userName}</span>`;
                            } else {
                                menuAccountElement.innerHTML = `<svg class="b3-menu__icon"><use xlink:href="#iconAccount"></use></svg><span class="b3-menu__label">${window.shehab.languages.login}</span>`;
                            }
                            processSync();
                        });
                        event.preventDefault();
...
<div class="fn__hr--b"></div>
<button id="login" class="b3-button fn__block">${window.shehab.languages.deactivateUser}</button>`;
    } else {
...
<div class="b3-form__space fn__none" id="form2">
    ...
    <div class="fn__hr--b"></div>
    <button id="login2" class="b3-button fn__block">${deactivate ? window.shehab.languages.deactivateUser : window.shehab.languages.login}</button>
</div>`;
};
...
const afterLogin = (response: IWebSocketData, deactive = false) => {
    if (deactive) {
        ...
        fetchPost("/api/setting/getCloudUser", {
            ...
            window.shehab.user = response.data;
            closePanel();
            document.getElementById("menuAccount").innerHTML = `<img class="b3-menu__icon" src="${window.shehab.user.userAvatarURL}"/>
<span class="b3-menu__label">${window.shehab.user.userName}</span>`;
            processSync();
        });
    }
};
...
const bindLoginEvent = (modelMainElement: HTMLElement, deactive = false) => {
    ...
    if (cloudRegionElement) {
        cloudRegionElement.addEventListener("change", () => {
            ...
            modelMainElement.querySelector("#form1").lastElementChild.innerHTML = `<a href="${getCloudURL("forget-pwd")}" class="b3-button b3-button--cancel" target="_blank">${window.shehab.languages.forgetPassword}</a>
        <span class="fn__space${window.shehab.config.system.container === "ios" ? " fn__none" : ""}"></span>
        <a href="${getCloudURL("register")}" class="b3-button b3-button--cancel${window.shehab.config.system.container === "ios" ? " fn__none" : ""}" target="_blank">${window.shehab.languages.register}</a>`;
        });
    }
...
export const login = () => {
    openModel({
        title: window.shehab.languages.login,
        icon: "iconAccount",
        html: getLoginHTML(),
        bindEvent(modelMainElement: HTMLElement) {
