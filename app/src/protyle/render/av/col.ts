...
export const getEditHTML = (options: {
    ...
    <span class="b3-menu__label ft__center">${window.shehab.languages.edit}</span>
</button>
    ...
    <div class="fn__block">
        <div class="fn__flex">
            ...
            <div class="b3-form__icona fn__block">
                ...
                <svg data-position="north" class="b3-form__icona-icon ariaLabel" aria-label="${colData.desc ? escapeAriaLabel(colData.desc) : window.shehab.languages.addDesc}"><use xlink:href="#iconInfo"></use></svg>
            </div>
        </div>
        <div class="fn__none">
            <div class="fn__hr"></div>
            <textarea placeholder="${window.shehab.languages.addDesc}" rows="1" data-type="desc" class="b3-text-field fn__block" type="text" data-value="${escapeAttr(colData.desc)}">${colData.desc}</textarea>
        </div>
        <div class="fn__hr--small"></div>
    </div>
</button>
<button class="b3-menu__item" data-type="goUpdateColType" ${colData.type === "block" ? "disabled" : ""}>
    <span class="b3-menu__label">${window.shehab.languages.type}</span>
...
<button class="b3-menu__item" data-type="nobg">
    ...
    <input data-type="addOption" class="b3-text-field fn__block" type="text" placeholder="${window.shehab.languages.enterKey} ${window.shehab.languages.addAttr}" style="margin: 4px 0">
</button>`;
        if (!colData.options) {
            colData.options = [];
        }
...
<button class="b3-menu__item" data-type="numberFormat" data-format="${colData.numberFormat}">
    ...
    <span class="b3-menu__label">${window.shehab.languages.format}</span>
    <span class="b3-menu__accelerator">${getLabelByNumberFormat(colData.numberFormat)}</span>
</button>`;
    } else if (colData.type === "template") {
        html += `<button class="b3-menu__separator" data-id="separator_2"></button>
<button class="b3-menu__item" data-type="nobg">
    <textarea spellcheck="false" rows="${Math.min(colData.template.split("\n").length, 8)}" placeholder="${window.shehab.languages.template}" data-type="updateTemplate" style="margin: 4px 0" rows="1" class="fn__block b3-text-field">${colData.template}</textarea>
</button>`;
...
<button class="b3-menu__item" data-type="goSearchAV" data-av-id="${colData.relation?.avID || ""}" data-old-value='${JSON.stringify(colData.relation || {})}'>
    <span class="b3-menu__label">${window.shehab.languages.relatedTo}</span>
    <span class="b3-menu__accelerator">${isSelf ? window.shehab.languages.thisDatabase : ""}</span>
    <svg class="b3-menu__icon b3-menu__icon--small"><use xlink:href="#iconRight"></use></svg>
</button>
<label class="b3-menu__item">
    <span class="fn__flex-center">${window.shehab.languages.backRelation}</span>
...
<div class="b3-menu__item fn__flex-column fn__none" data-type="nobg">
    <button style="margin: 4px 0 8px;" class="b3-button fn__block" data-type="updateRelation">${window.shehab.languages.confirm}</button>
</div>`;
        ...
        html += `<button class="b3-menu__separator" data-id="separator_2"></button>
<label class="b3-menu__item">
    <span class="fn__flex-center">${window.shehab.languages.fillCreated}</span>
    <span class="fn__space fn__flex-1"></span>
    <input data-type="fillCreated" type="checkbox" class="b3-switch b3-switch--menu" ${colData.date?.autoFillNow ? "checked" : ""}>
</label>
<label class="b3-menu__item">
    <span class="fn__flex-center">${window.shehab.languages.fillSpecificTime}</span>
    <span class="fn__space fn__flex-1"></span>
    <input data-type="fillSpecificTime" type="checkbox" class="b3-switch b3-switch--menu" ${colData.date?.fillSpecificTime ? "checked" : ""}>
        ...
        html += `<button class="b3-menu__separator" data-id="separator_2"></button>
<label class="b3-menu__item">
    <span class="fn__flex-center">${window.shehab.languages.includeTime}</span>
    <span class="fn__space fn__flex-1"></span>
...
<label class="b3-menu__item">
    ...
    <span class="fn__flex-center">${window.shehab.languages.wrap}</span>
    <span class="fn__space fn__flex-1"></span>
...
export const bindEditEvent = (options: {
    ...
    descElement.addEventListener("input", () => {
        nameElement.nextElementSibling.setAttribute("aria-label", descElement.value ? escapeHtml(descElement.value) : window.shehab.languages.addDesc);
    });
    ...
    if (backRelationElement) {
        ...
        if (oldValue.avID) {
            fetchPost("/api/av/getAttributeView", {id: oldValue.avID}, (response) => {
                goSearchElement.querySelector(".b3-menu__accelerator").textContent = oldValue.avID === avID ? window.shehab.languages.thisDatabase : (response.data.av.name || window.shehab.languages._kernel[267]);
                response.data.av.keyValues.find((item: { key: { id: string, name: string } }) => {
                    if (item.key.id === oldValue.backKeyID) {
                        inputElement.setAttribute("data-old-value", item.key.name || window.shehab.languages._kernel[272]);
                        inputElement.value = item.key.name || window.shehab.languages._kernel[272];
                        return true;
                    }
                });
                toggleUpdateRelationBtn(options.menuElement, avID);
...
export const getColNameByType = (type: TAVCol) => {
    switch (type) {
        ...
        case "email":
        case "template":
            return window.shehab.languages[type];
        case "mSelect":
            return window.shehab.languages.multiSelect;
        case "relation":
            return window.shehab.languages.relation;
        case "rollup":
            return window.shehab.languages.rollup;
        case "updated":
            return window.shehab.languages.updatedTime;
        case "created":
            return window.shehab.languages.createdTime;
        case "url":
            return window.shehab.languages.link;
        case "mAsset":
            return window.shehab.languages.assets;
        case "checkbox":
            return window.shehab.languages.checkbox;
        case "block":
            return window.shehab.languages["_attrView"].key;
        case "lineNumber":
            return window.shehab.languages.lineNumber;
    }
};
...
export const showColMenu = (protyle: IProtyle, blockElement: Element, cellElement: HTMLElement) => {
    ...
    <div class="b3-form__icona fn__block">
        ...
        <svg data-position="north" class="b3-form__icona-icon ariaLabel" aria-label="${oldDesc ? escapeAriaLabel(oldDesc) : window.shehab.languages.addDesc}"><use xlink:href="#iconInfo"></use></svg>
    </div>
    <div class="fn__space"></div>
</div>
<div class="fn__none">
    ...
    <div class="fn__flex">
        <span class="fn__space"></span>
        <textarea placeholder="${window.shehab.languages.addDesc}" rows="1" class="b3-text-field fn__block" type="text" data-value="${escapeAttr(oldDesc)}">${oldDesc}</textarea>
        <span class="fn__space"></span>    
    </div>
</div>
<div class="fn__hr--small"></div>`,
    ...
    menu.addItem({
        id: "edit",
        icon: "iconEdit",
        label: window.shehab.languages.edit,
        click() {
    ...
    if (type !== "lineNumber") {
        menu.addItem({
            id: "filter",
            icon: "iconFilter",
            label: window.shehab.languages.filter,
            click() {
        ...
        menu.addItem({
            id: "asc",
            icon: "iconUp",
            label: window.shehab.languages.asc,
            click() {
                fetchPost("/api/av/renderAttributeView", {
                    id: avID,
                }, (response) => {
        ...
        menu.addItem({
            id: "desc",
            icon: "iconDown",
            label: window.shehab.languages.desc,
            click() {
                fetchPost("/api/av/renderAttributeView", {
                    id: avID,
                }, (response) => {
    ...
    menu.addItem({
        ...
        label: isPin ? window.shehab.languages.unfreezeCol : window.shehab.languages.freezeCol,
        click() {
            transaction(protyle, [{
    ...
    if (type !== "block") {
        menu.addItem({
            id: "hide",
            icon: "iconEyeoff",
            label: window.shehab.languages.hide,
            click() {
                transaction(protyle, [{
    ...
    menu.addItem({
        icon: "iconRefresh",
        label: window.shehab.languages.syncColWidth,
        click() {
            transaction(protyle, [{
    ...
    menu.addItem({
        icon: "iconSoftWrap",
        label: `<label class="fn__flex fn__pointer"><span>${window.shehab.languages.wrap}</span><span class="fn__space fn__flex-1"></span>
<input type="checkbox" class="b3-switch b3-switch--menu"${cellElement.dataset.wrap === "true" ? " checked" : ""}></label>`,
                ...
                if (type === "relation") {
                    ...
                    if (colData.key.relation?.isTwoWay) {
                        ...
                        const dialog = new Dialog({
                            title: window.shehab.languages.removeColConfirm,
                            content: `<div class="b3-dialog__content">
    ${window.shehab.languages.confirmRemoveRelationField
                                .replace("${x}", colData.key.name || window.shehab.languages._kernel[272])
                                .replace("${y}", relResponse.data.av.name || window.shehab.languages._kernel[267])
                                .replace("${z}", relResponse.data.av.keyValues.find((item: {
                                    key: { id: string }
                                }) => item.key.id === colData.key.relation.backKeyID).key.name || window.shehab.languages._kernel[272])}
    <div class="fn__hr--b"></div>
    <button class="fn__block b3-button b3-button--remove" data-action="delete">${window.shehab.languages.removeBothRelationField}</button>
    <div class="fn__hr"></div>
    <button class="fn__block b3-button b3-button--remove" data-action="keep-relation">${window.shehab.languages.removeButKeepRelationField}</button>
    <div class="fn__hr"></div>
    ...
    const inputElement = window.shehab.menus.menu.element.querySelector(".b3-text-field") as HTMLInputElement;
    if (inputElement) {
        inputElement.select();
        inputElement.focus();
    }
...
export const addCol = (protyle: IProtyle, blockElement: Element, previousID?: string) => {
    ...
    menu.addItem({
        ...
        click() {
            ...
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.text,
                avID,
                type: "text",
                id,
                previousID
    ...
    menu.addItem({
        id: "number",
        icon: "iconNumber",
        label: window.shehab.languages.number,
        click() {
            const id = Lute.NewNodeID();
            const newUpdated = dayjs().format("YYYYMMDDHHmmss");
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.number,
                avID,
                type: "number",
                id,
                previousID
    ...
    menu.addItem({
        id: "select",
        icon: "iconListItem",
        label: window.shehab.languages.select,
        click() {
            const id = Lute.NewNodeID();
            const newUpdated = dayjs().format("YYYYMMDDHHmmss");
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.select,
                avID,
                type: "select",
                id,
                previousID
    ...
    menu.addItem({
        id: "multiSelect",
        icon: "iconList",
        label: window.shehab.languages.multiSelect,
        click() {
            const id = Lute.NewNodeID();
            const newUpdated = dayjs().format("YYYYMMDDHHmmss");
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.multiSelect,
                avID,
                type: "mSelect",
                id,
                previousID
    ...
    menu.addItem({
        id: "date",
        icon: "iconCalendar",
        label: window.shehab.languages.date,
        click() {
            const id = Lute.NewNodeID();
            const newUpdated = dayjs().format("YYYYMMDDHHmmss");
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.date,
                avID,
                type: "date",
                id,
                previousID
    ...
    menu.addItem({
        ...
        click() {
            ...
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.assets,
                avID,
                type: "mAsset",
                id,
                previousID
    ...
    menu.addItem({
        id: "checkbox",
        icon: "iconCheck",
        label: window.shehab.languages.checkbox,
        click() {
            const id = Lute.NewNodeID();
            const newUpdated = dayjs().format("YYYYMMDDHHmmss");
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.checkbox,
                avID,
                type: "checkbox",
                id,
                previousID
    ...
    menu.addItem({
        ...
        click() {
            ...
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.link,
                avID,
                type: "url",
                id,
                previousID
    ...
    menu.addItem({
        id: "email",
        icon: "iconEmail",
        label: window.shehab.languages.email,
        click() {
            const id = Lute.NewNodeID();
            const newUpdated = dayjs().format("YYYYMMDDHHmmss");
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.email,
                avID,
                type: "email",
                id,
                previousID
    ...
    menu.addItem({
        id: "phone",
        icon: "iconPhone",
        label: window.shehab.languages.phone,
        click() {
            const id = Lute.NewNodeID();
            const newUpdated = dayjs().format("YYYYMMDDHHmmss");
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.phone,
                avID,
                type: "phone",
                id,
                previousID
    ...
    menu.addItem({
        ...
        click() {
            ...
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.template,
                avID,
                type: "template",
                id,
                previousID
    ...
    menu.addItem({
        id: "relation",
        icon: "iconOpen",
        label: window.shehab.languages.relation,
        click() {
            const id = Lute.NewNodeID();
            const newUpdated = dayjs().format("YYYYMMDDHHmmss");
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.relation,
                avID,
                type: "relation",
                id,
                previousID
    ...
    menu.addItem({
        id: "rollup",
        icon: "iconSearch",
        label: window.shehab.languages.rollup,
        click() {
            const id = Lute.NewNodeID();
            const newUpdated = dayjs().format("YYYYMMDDHHmmss");
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.rollup,
                avID,
                type: "rollup",
                id,
                previousID
    ...
    menu.addItem({
        ...
        label: window.shehab.languages.lineNumber,
        click() {
            const id = Lute.NewNodeID();
            const newUpdated = dayjs().format("YYYYMMDDHHmmss");
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.lineNumber,
                avID,
                type: "lineNumber",
                id,
                previousID
    ...
    menu.addItem({
        id: "createdTime",
        icon: "iconClock",
        label: window.shehab.languages.createdTime,
        click() {
            const id = Lute.NewNodeID();
            const newUpdated = dayjs().format("YYYYMMDDHHmmss");
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.createdTime,
                avID,
                type: "created",
                id,
                previousID
    ...
    menu.addItem({
        id: "updatedTime",
        icon: "iconClock",
        label: window.shehab.languages.updatedTime,
        click() {
            const id = Lute.NewNodeID();
            const newUpdated = dayjs().format("YYYYMMDDHHmmss");
            transaction(protyle, [{
                action: "addAttrViewCol",
                name: window.shehab.languages.updatedTime,
                avID,
                type: "updated",
                id,
                previousID
