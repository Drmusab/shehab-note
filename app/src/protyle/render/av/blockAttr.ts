...
const genAVRollupHTML = (value: IAVCellValue) => {
    ...
    switch (value.type) {
        case "block":
            if (value?.isDetached) {
                html = `<span>${value.block?.content || window.shehab.languages.untitled}</span>`;
            } else {
                html = `<span data-type="block-ref" data-id="${value.block.id}" data-subtype="s" class="av__celltext--ref">${value.block?.content || window.shehab.languages.untitled}</span>`;
            }
            break;
        case "text":
...
export const genAVValueHTML = (value: IAVCellValue) => {
    ...
    switch (value.type) {
        case "block":
            html = `<input data-id="${value.block.id}" value="${escapeAttr(value.block.content)}" type="text" class="b3-text-field b3-text-field--text fn__flex-1" placeholder="${window.shehab.languages.empty}">`;
            break;
        case "text":
            html = `<textarea style="resize: vertical" rows="${(value.text?.content || "").split("\n").length}" class="b3-text-field b3-text-field--text fn__flex-1" placeholder="${window.shehab.languages.empty}">${value.text?.content || ""}</textarea>`;
            break;
        case "number":
            html = `<input value="${value.number.isNotEmpty ? value.number.content : ""}" type="number" class="b3-text-field b3-text-field--text fn__flex-1" placeholder="${window.shehab.languages.empty}">
<span class="fn__space"></span><span class="fn__flex-center ft__on-surface b3-tooltips__w b3-tooltips" aria-label="${window.shehab.languages.format}">${value.number.formattedContent}</span><span class="fn__space"></span>`;
            break;
        case "mSelect":
        case "select":
        ...
        case "mAsset":
            ...
            break;
        case "date":
            html = `<span class="av__celltext" data-value='${JSON.stringify(value[value.type])}' placeholder="${window.shehab.languages.empty}">`;
        ...
        case "updated":
            ...
            break;
        case "url":
            html = `<input value="${value.url.content}" class="b3-text-field b3-text-field--text fn__flex-1" placeholder="${window.shehab.languages.empty}">
<span class="fn__space"></span>
<a ${value.url.content ? `href="${value.url.content}"` : ""} target="_blank" aria-label="${window.shehab.languages.openBy}" class="block__icon block__icon--show fn__flex-center b3-tooltips__w b3-tooltips"><svg><use xlink:href="#iconLink"></use></svg></a>`;
            break;
        case "phone":
            html = `<input value="${value.phone.content}" class="b3-text-field b3-text-field--text fn__flex-1" placeholder="${window.shehab.languages.empty}">
<span class="fn__space"></span>
<a ${value.phone.content ? `href="tel:${value.phone.content}"` : ""} target="_blank" aria-label="${window.shehab.languages.openBy}" class="block__icon block__icon--show fn__flex-center b3-tooltips__w b3-tooltips"><svg><use xlink:href="#iconPhone"></use></svg></a>`;
            break;
        case "checkbox":
            html = `<svg class="av__checkbox"><use xlink:href="#icon${value.checkbox.checked ? "Check" : "Uncheck"}"></use></svg>`;
            break;
        case "template":
            html = `<div class="fn__flex-1" placeholder="${window.shehab.languages.empty}">${value.template.content}</div>`;
            break;
        case "email":
            html = `<input value="${value.email.content}" class="b3-text-field b3-text-field--text fn__flex-1" placeholder="${window.shehab.languages.empty}">
<span class="fn__space"></span>
<a ${value.email.content ? `href="mailto:${value.email.content}"` : ""} target="_blank" aria-label="${window.shehab.languages.openBy}" class="block__icon block__icon--show fn__flex-center b3-tooltips__w b3-tooltips"><svg><use xlink:href="#iconEmail"></use></svg></a>`;
            break;
        case "relation":
            value?.relation?.contents?.forEach((item, index) => {
                if (item && item.block) {
                    ...
                    if (item?.isDetached) {
                        html += `<span data-row-id="${rowID}" class="av__cell--relation"><span>➖<span class="fn__space--5"></span></span><span class="av__celltext">${Lute.EscapeHTMLStr(item.block.content || window.shehab.languages.untitled)}</span></span>`;
                    } else {
                        // data-block-id 用于更新 emoji
                        html += `<span data-row-id="${rowID}" class="av__cell--relation" data-block-id="${item.block.id}"><span class="b3-menu__avemoji" data-unicode="${item.block.icon || ""}">${unicode2Emoji(item.block.icon || window.shehab.storage[Constants.LOCAL_IMAGES].file)}</span><span data-type="block-ref" data-id="${item.block.id}" data-subtype="s" class="av__celltext av__celltext--ref">${Lute.EscapeHTMLStr(item.block.content || window.shehab.languages.untitled)}</span></span>`;
                    }
                }
            });
...
export const renderAVAttribute = (element: HTMLElement, id: string, protyle: IProtyle, cb?: (element: HTMLElement) => void) => {
    ...
    <div class="block__logo popover__block" style="max-width:calc(100% - 40px)" data-id='${JSON.stringify(table.blockIDs)}'>
        ...
        <span class="fn__ellipsis">${table.avName || window.shehab.languages.database}</span>
    </div>
    <div class="fn__flex-1"></div>
    <span data-type="remove" data-row-id="${table.keyValues && table.keyValues[0].values[0].blockID}" class="block__icon block__icon--warning block__icon--show b3-tooltips__w b3-tooltips" aria-label="${window.shehab.languages.removeAV}"><svg><use xlink:href="#iconTrashcan"></use></svg></span>
</div>`;
...
${["text", "number", "date", "url", "phone", "template", "email"].includes(item.values[0].type) ? "" : `placeholder="${window.shehab.languages.empty}"`}  
...
<button data-type="addColumn" class="b3-button b3-button--cancel"><svg><use xlink:href="#iconAdd"></use></svg>${window.shehab.languages.newCol}</button>
<div class="fn__hr--b"></div><div class="fn__hr--b"></div>`;
        ...
        if (element.innerHTML === "") {
            ...
            element.addEventListener("dragstart", (event: DragEvent) => {
                ...
                window.shehab.dragElement = target.parentElement;
                window.shehab.dragElement.style.opacity = ".38";
                dragBlockElement = hasClosestBlock(window.shehab.dragElement) as HTMLElement;
            ...
            element.addEventListener("drop", (event) => {
                ...
                if (targetElement && dragBlockElement) {
                    ...
                    const undoPreviousID = window.shehab.dragElement.previousElementSibling?.getAttribute("data-col-id");
                    if (previousID !== undoPreviousID && previousID !== window.shehab.dragElement.dataset.colId) {
                        transaction(protyle, [{
                            action: "sortAttrViewKey",
                            avID: dragBlockElement.dataset.avId,
                            previousID,
                            id: window.shehab.dragElement.dataset.colId,
                        }], [{
                            action: "sortAttrViewKey",
                            avID: dragBlockElement.dataset.avId,
                        ...
                        if (isBottom) {
                            targetElement.after(window.shehab.dragElement);
                        } else {
                            targetElement.before(window.shehab.dragElement);
                        }
                    }
                    targetElement.classList.remove("dragover__bottom", "dragover__top");
                } else if (!window.shehab.dragElement && event.dataTransfer.types[0] === "Files") {
            ...
            element.addEventListener("dragover", (event: DragEvent) => {
                ...
                if (!targetElement || targetElement === window.shehab.dragElement || !dragBlockElement) {
                    return;
                }
    ...
    while (target && element !== target) {
        ...
        if (target.classList.contains("b3-menu__avemoji")) {
            ...
            openEmojiPanel(target.nextElementSibling.getAttribute("data-id"), "doc", {
                ...
                target.innerHTML = unicode2Emoji(unicode || window.shehab.storage[Constants.LOCAL_IMAGES].file);
