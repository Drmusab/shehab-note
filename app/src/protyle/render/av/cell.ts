...
export const popTextCell = (protyle: IProtyle, cellElements: HTMLElement[], type?: TAVCol) => {
    ...
    document.body.insertAdjacentHTML("beforeend", `<div class="av__mask" style="z-index: ${++window.shehab.zIndex}">
    ${html}
</div>`);
...
export const renderCell = (cellValue: IAVCellValue, rowIndex = 0, showIcon = true, type: TAVView = "table") => {
    ...
    if ("template" === cellValue.type) {
        ...
        if (cellValue?.isDetached) {
            text = `<span class="av__celltext">${Lute.EscapeHTMLStr(cellValue.block.content || "")}</span><span class="b3-chip b3-chip--info b3-chip--small" data-type="block-more">${window.shehab.languages.more}</span>`;
        } else {
            text = `<span class="b3-menu__avemoji${showIcon ? "" : " fn__none"}" data-unicode="${cellValue.block.icon || ""}">${unicode2Emoji(cellValue.block.icon || window.shehab.storage[Constants.LOCAL_IMAGES].file)}</span><span data-type="block-ref" data-id="${cellValue.block.id}" data-subtype="s" class="av__celltext av__celltext--ref">${Lute.EscapeHTMLStr(cellValue.block.content)}</span><span class="b3-chip b3-chip--info b3-chip--small" data-type="block-more">${window.shehab.languages.update}</span>`;
        }
        ...
        cellValue?.relation?.contents?.forEach((item, index) => {
            if (item && item.block) {
                ...
                if (item?.isDetached) {
                    text += `<span data-row-id="${rowID}" class="av__cell--relation"><span class="${showIcon ? "" : " fn__none"}">➖<span class="fn__space--5"></span></span><span class="av__celltext">${Lute.EscapeHTMLStr(item.block.content || window.shehab.languages.untitled)}</span></span>`;
                } else {
                    // data-block-id 用于更新 emoji
                    text += `<span data-row-id="${rowID}" class="av__cell--relation" data-block-id="${item.block.id}"><span class="b3-menu__avemoji${showIcon ? "" : " fn__none"}" data-unicode="${item.block.icon || ""}">${unicode2Emoji(item.block.icon || window.shehab.storage[Constants.LOCAL_IMAGES].file)}</span><span data-type="block-ref" data-id="${item.block.id}" data-subtype="s" class="av__celltext av__celltext--ref">${Lute.EscapeHTMLStr(item.block.content || window.shehab.languages.untitled)}</span></span>`;
                }
            }
        });
...
const renderRollup = (cellValue: IAVCellValue, showIcon: boolean) => {
    ...
    if (["text"].includes(cellValue.type)) {
        ...
        if (cellValue?.isDetached) {
            text = `<span class="av__celltext">${Lute.EscapeHTMLStr(cellValue.block?.content || window.shehab.languages.untitled)}</span>`;
        } else {
            text = `<span class="b3-menu__avemoji${showIcon ? "" : " fn__none"}" data-unicode="${cellValue.block.icon || ""}">${unicode2Emoji(cellValue.block.icon || window.shehab.storage[Constants.LOCAL_IMAGES].file)}</span><span data-type="block-ref" data-id="${cellValue.block?.id}" data-subtype="s" class="av__celltext av__celltext--ref">${Lute.EscapeHTMLStr(cellValue.block?.content || window.shehab.languages.untitled)}</span>`;
        }
    } else if (cellValue.type === "number") {
...
export const addDragFill = (cellElement: Element) => {
    ...
    if (!cellElement.querySelector(".av__drag-fill")) {
        ...
        cellElement.insertAdjacentHTML("beforeend", `<div aria-label="${window.shehab.languages.dragFill}" class="av__drag-fill ariaLabel"></div>`);
    }
};
