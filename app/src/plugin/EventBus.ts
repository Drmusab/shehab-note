...
export const emitOpenMenu = (options: {
    ...
    if (pluginSubMenu.menus.length > 0) {
        if (options.separatorPosition === "top") {
            window.shehab.menus.menu.append(new MenuItem({id: "separator_pluginTop", type: "separator"}).element);
        }
        window.shehab.menus.menu.append(new MenuItem({
            id: "plugin",
            label: window.shehab.languages.plugin,
            icon: "iconPlugin",
            type: "submenu",
            submenu: pluginSubMenu.menus,
        }).element);
        if (options.separatorPosition === "bottom") {
            window.shehab.menus.menu.append(new MenuItem({id: "separator_pluginBottom", type: "separator"}).element);
        }
    }
};
