        ...
        if (plugin.name === name) {
            ...
            docksKeys.forEach(key => {
                if (window.shehab.storage[Constants.LOCAL_PLUGIN_DOCKS][plugin.name] && window.shehab.storage[Constants.LOCAL_PLUGIN_DOCKS][plugin.name][key]) {
                    window.shehab.storage[Constants.LOCAL_PLUGIN_DOCKS][plugin.name][key].show =
                        !!document.querySelector(`.dock__item[data-type="${key}"]`)?.classList.contains("dock__item--active");
                }
                if (Object.keys(window.shehab.layout.leftDock.data).includes(key)) {
                    window.shehab.layout.leftDock.remove(key);
                } else if (Object.keys(window.shehab.layout.rightDock.data).includes(key)) {
                    window.shehab.layout.rightDock.remove(key);
                } else if (Object.keys(window.shehab.layout.bottomDock.data).includes(key)) {
                    window.shehab.layout.bottomDock.remove(key);
                }
            });
            // rm listen
